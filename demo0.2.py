...
#print(image)
	...
	iCPU = s.get('https://192.168.1.100'+ image)
	iMemory = s.get('https://192.168.1.100' + '/Orion/Netperfmon/Chart.aspx?ChartName=HostAvgPercentMemoryUsed&Title=&SubTitle=&SubTitle2=&Width=640&Height=0&SampleSize=30M&Period=LAST 7 DAYS&ShowTrend=True&FontSize=1&NetObject=N:' + ipcode,  verify=False)

#print(iCPU)
	imgCPU = iCPU.content
	imgMemory = iMemory.content
#print(iCPU.text)
	with open('D:/test/img/'+ str(ip) + '- CPU - last7day.png', 'wb') as fCPU:
		fCPU.write(imgCPU)
		print("完成设备" + str(ip) + "的CPU图片下载!\n")
		time.sleep(5)
	with open('D:/test/img/' + str(ip) + ' - memory - last7day.png', 'wb') as fMemory:
		fMemory.write(imgMemory)
		print("完成设备" + str(ip) + "的Memory图片下载!\n")
		time.sleep(6)
print("全部下载完成！")
